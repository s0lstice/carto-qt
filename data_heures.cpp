...
bool initHeuresTable(){
    ...
    if(query.exec("create table heures (heure_id INTEGER PRIMARY KEY AUTOINCREMENT,"
                  "point_id INTEGER,"
                  "jour INTEGER,"
                  "debut HEURE,"
                  "fin DATE,"
                  "FOREIGN KEY(point_id) REFERENCES points(point_id))") == false)
    {
        qDebug()<< "iniHeuresTable" << query.lastError().text();
        return false;
    }
    return true;
}
